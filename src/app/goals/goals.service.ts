...
export interface IGoalsService {
    ...
    incrementCurrentQuantity(quantity: number): Promise<Goals>
    changeExpectedQuantity(quantity: number): Promise<Goals>
...
export class GoalsService implements IGoalsService {
    ...
    async incrementCurrentQuantity(quantity: number): Promise<Goals> {
        const savedGoal = await this.create(new CreateGoalDTO(0, 0));
        const updateGoalDTO = new UpdateGoalDTO();
        ...
        return await this.update(savedGoal.id, updateGoalDTO);
    }

    async changeExpectedQuantity(quantity: number): Promise<Goals> {
        const savedGoal = await this.create(new CreateGoalDTO(0, 0));
        const updateGoalDTO = new UpdateGoalDTO();
